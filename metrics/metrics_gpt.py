...
class COPAMetric(MetricBase):
    def __init__(self, pred=None, target=None, seq_len=None, tokenizer=None):
        ...
        if tokenizer is None:
            tokenizer = RobertaTokenizer.from_pretrained('roberta-large')
        self.label_map = {
            tokenizer.encode('0', add_special_tokens=False)[0]: 0,  # negative
            tokenizer.encode('1', add_special_tokens=False)[0]: 1,  # positive
        }
